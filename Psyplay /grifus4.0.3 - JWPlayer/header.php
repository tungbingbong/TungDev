...
<?php } ?>
<?php if($values = get_post_custom_values("fondo_player")) { ?><meta property="og:image" content="<?php echo $values[0]; ?>" /><?php } ?>
...
<style type="text/css">
.buscaicon ul li a.buscaboton {background-color:#<?php echo $color; ?>}
.iteslid ul li a.selected, .filtro_y ul li a:hover {background:#<?php echo $color; ?>}
.news_home .noticias .new .fecha .mes, .categorias li:hover:before {color:#<?php echo $color; ?>}
#header .navegador .caja .menu li.current-menu-item a, #slider1 .item .imagens span.imdb b, #slider2 .item .imagens span.imdb b, .items .item .image span.imdb b, .items .item .boxinfo .typepost, #contenedor .contenido .header .buscador .imputo:before, .categorias li.current-cat:before {color:#<?php echo $color; ?>}
</style>
<?php } } else { ?>
<?php 
...
<style>
#header .navegador, .rheader {background:#<?php echo $color2; ?>;}
#header .navegador .caja .menu li.current-menu-item a {color:#<?php echo $color1; ?>;}
#header .navegador .caja .menu li a:hover {background:<?php echo $color3; ?>;}
.buscaicon ul li a.buscaboton, .categorias li span, .iteslid ul li a.selected, .imdb_r .a span {background-color:#<?php echo $color1; ?>;}
.news_home .noticias .new .fecha .dia, #header .navegador .caja .menu ul li ul.sub-menu li a:before,  .box_item h1 {color: #<?php echo $color1; ?>;}
</style>
<?php } } ?>
</head>
<body id="bodyplus">
...
<?php if($dato = get_option('twt_url')) { ?><li><a href="<?php echo $dato; ?>" target="_blank" class="twtr">Twitter</a></li><?php } ?>
<?php if($dato = get_option('gogl_url')) { ?><li><a href="<?php echo $dato; ?>" target="_blank" class="gge">Google +</a></li><?php } ?>
</ul>
</li>
<?php } ?>
...
<a href="<?php bloginfo('url'); ?>/wp-admin/post-new.php" target="_blank"><?php _e('Add new post', 'mundothemes'); ?></a>
<?php endif; ?>
<?php endif; ?>
...
<?php } else { ?>
<a href="javascript:void(0)" onclick="document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'"><?php _e('Login / Registration','mundothemes'); ?></a>
<?php } ?>
<?php } else { ?>
<?php } ?>
</div>
...
<form method="get" id="searchform" action="<?php bloginfo('url'); ?>">
<input type="text" placeholder="<?php _e('Search..', 'mundothemes'); ?>" name="s" id="s" value="<?php echo $_GET['s'] ?>">
</form>
</div>