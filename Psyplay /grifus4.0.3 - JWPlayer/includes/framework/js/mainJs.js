...
jQuery(document).ready(function() {
    ...
    jQuery('input[type="checkbox"]').each(function() {
        var $this = jQuery(this);
        var id = $this.attr("id");
        if (!$this.is(':checked')) jQuery('div[rel="' + id + '"]').hide()
    });
    jQuery('input[type="checkbox"]').click(function() {
        var $this = jQuery(this);
        var id = $this.attr("id");
        if ($this.is(':checked')) jQuery('div[rel="' + id + '"]').slideFadeToggle(500);
        else jQuery('div[rel="' + id + '"]').slideFadeToggle(500)
    });
    jQuery('.acera_upload').each(function() {
        ...
        new AjaxUpload(image_id, {
            ...
            onComplete: function(file, response) {
                the_button.html("Upload");
                if (response.search("Error") > -1) {
                    alert("There was an error uploading:\n" + response)
                } else {
                    image_input.val(response);
                    var image_preview = '<img src="' + response + '" />';
                    var remove_button_id = 'remove_' + image_id;
                    var rem_id = "#" + remove_button_id;
                    if (!(jQuery(rem_id).length > 0)) {
                        the_button.after('<span class="acera_remove acera-button" id="' + remove_button_id + '">Remove</span>')
                    }
