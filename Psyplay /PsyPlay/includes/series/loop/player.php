...
<div class="les-content"><a href="#tab-ad"><?php if ($note = get_option('quality_adplayer') ) { echo $note; } else { echo 'Drive Google' ;} ?></a></div>
</li>
<?php }?>
<?php $numerado = 1; { while( have_rows('player') ): the_row(); ?>
<li><div class="les-title">
...
<div id="pbar_outerdiv" class="movieplay">
<?php if($values = get_post_custom_values("fondo_player")) { ?>
...
$values = str_replace( ':','',  $values);
echo '<iframe src="https://putstream.com/show/'.$values.'/';
...
<li><div class="les-title">
<i class="fa fa-server mr5"></i>
<strong><?php if ($note = get_option('server_adplayer') ) { echo $note; } else { echo 'HD Server Openload' ;} ?></strong>

</div>
<div class="les-content"><a href="#tab-ad"><?php if ($note = get_option('quality_adplayer') ) { echo $note; } else { echo 'HD Openload 1080p' ;} ?></a></div>
</li>
<li>
<div class="les-title">
<i class="fa fa-server mr5"></i>
...
$seson = info_movie_get_meta( 'temporada' );
$epi = info_movie_get_meta( 'episodio' );
echo '<iframe src="https://freemovietube.tv/'.$values.'-'.$seson.'-'.$epi.'-openload.html" width="1280" height="720" allowscriptaccess="always" allowfullscreen="true" scrolling="no" frameborder="0"></iframe>';
?>
