...
class UserController {
    ...
    userProfile(req, res, next){
        ...
        User.findById(id)
            ...
            user.save();
            res.render('userProfile', {user : mongooseToObj(user),token: user.resetPasswordToken })
        })
        .catch(next);
    }
    ...
    order(req, res, next){
        ...
        const userOrder = new UserOrder({
            ...
            address: address,
            payment: payment,
            dateOrder: d.toString().slice(0,24),
    ...
    checkout(req, res, next){
        ...
        User.findById(id)
            .then(user => {
            res.render('checkout', {user : mongooseToObj(user), products: cart.generateArray(), 
                totalPrice: cart.totalPrice.toFixed(2), userID:id})
        })
        .catch(next);
    }
