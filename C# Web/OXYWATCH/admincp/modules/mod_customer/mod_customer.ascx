<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_customer.ascx.cs" Inherits="admincp_modules_mod_customer_mod_customer" %>
...
<script type="text/javascript">
    function checkSubmitActiveInactiveForm(field, strAction) {
        ...
        if (!isChecked) {
            ...
            if (confirm('Ban da chac chan thuc hien tac vu nay chua?')) {
                ...
                field.action = "Default.aspx?page=customer&mod=customer&do=" + strAction;
                field.submit();
                return true;
            }
            else
    ...
    function checkFormSearch(field) {
        ...
        */
        ...
        obj.style.visibility = (obj.style.visibility == 'visible') ? 'hidden' : 'visible';
        //=================================================
        field.action = "Default.aspx?page=customer&mod=customer&do=search&key=" + field.key.value + "&per=" + field.per.value;
        field.submit();
        return true;
    }
			...
			<tr>
				...
				<table class="content">
					...
					<tr>
						...
						<table class="search">
							<tr>
								...
								<%--<%= clsHtml.add(2, "Default.aspx?page=add_edit_customer&mod=customer&do=add")%>	--%>								
								</td>
							</tr>
						</table>
						</div>
					...
					<%	
                        ...
                        //}
                        //------------------------------ 		    
                        string strSqlCount = "select count(*) from tbl_customer where (FK_LangID = 1 or FK_LangID = 2)" + strSearch;
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select * from tbl_customer where  (FK_LangID = 1 or FK_LangID = 2)" + strSearch + " order by C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light">
								...
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_customer", "") > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                ...
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_customer", "") < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								...
								<%--&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_customer&mod=customer&do=edit&id=" + dt.Rows[i]["PK_customerID"].ToString())%> 
								--%>
                                &nbsp;								
