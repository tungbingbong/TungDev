<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_product_price.ascx.cs" Inherits="admincp_modules_mod_product_mod_product_price" %>
...
<script type="text/javascript">
    function checkSubmitActiveInactiveForm(field, strAction) {
        ...
        if (!isChecked) {
            ...
            if (confirm('Ban da chac chan thuc hien tac vu nay chua?')) {
                ...
                field.action = "Default.aspx?page=product_price&mod=product&do=" + strAction;
                field.submit();
                return true;
            }
            else
    ...
    function checkFormSearch(field) {
        ...
        */
        field.action = "Default.aspx?page=product_price&mod=product&do=search&key=" + field.key.value + "&per=" + field.per.value;
        field.submit();
        return true;
    }
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<td width="95%"><span class="title">Quản lý báo giá sản phẩm</span></td>
				</tr>    		
			</table>
				<table class="content">
					<tr>
						...
						<table class="header" cellpadding="0" cellspacing="0">
							<tr>
								<th>Danh sách báo giá sản phẩm</th>
							</tr>	
						</table>
						</td>
					</tr>
					<tr>
						...
						<table class="search">
							<tr>
								...
								<%= clsHtml.add(2, "Default.aspx?page=add_edit_product_price&mod=product&do=add")%>									
								</td>
							</tr>
						</table>
						</div>
					...
					<%	
                        ...
                        string strSearch = "";
                        if (strKey != null)
                            strSearch = " and (tbl_product_price.C_Name like '%" + strKey + "%' )";                        		    
                        //------------------------------ 		    
                        string strSqlCount = "select count(*) from tbl_product_price where tbl_product_price.FK_LangID = " + lang.getLangID() + strSearch;
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select * from tbl_product_price where tbl_product_price.FK_LangID = " + lang.getLangID() + strSearch + " order by tbl_product_price.C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light">
								<td align="center">
								<% Response.Write("<input name=\"row[]\" type=\"checkbox\" class=\"input_checkbox\" id=\"row[]\" value=\"" + dt.Rows[i]["PK_ProductPriceID"].ToString() + "\" />"); %>								
								</td>
								<td align="center">		
																		
								<a href="<%=clsConfig.GetHostUrl() %>/<%=dt.Rows[i]["C_Attach"].ToString() %>">File báo giá</a>
								</td>								
								<td align="Left">														
								&nbsp;
								...
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_product_price", "tbl_product_price.FK_LangID =" + lang.getLangID()) > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=up&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString() + "'>"); %>
								    <img src="images/arrow-up.gif"  onmouseover="Tip('Di chuyển lên')" border="0">
								    <%= "</a>" %>
		                        <%
                                    }		                         
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_product_price", "tbl_product_price.FK_LangID =" + lang.getLangID()) < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=down&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString() + "'>"); %>
								...
								<% if ((int)dt.Rows[i]["C_Active"] == 0)
                                   {
                                %>
								   <%= clsHtml.UnLock(2, clsConfig.getCurrentUrl() + "&do=unlock&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString())%>
		                        <%
                                    }
                                    else
                                    {		                         
                                %>
                                <%= clsHtml.Lock(2, clsConfig.getCurrentUrl() + "&do=lock&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString())%>
								<% } %>									
								&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_product_price&mod=product&do=edit&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString())%> 
								&nbsp;								
								<%= clsHtml.del(2, clsConfig.getCurrentUrl() + "&do=delete&id=" + dt.Rows[i]["PK_ProductPriceID"].ToString())%>    							
								</td>	
							</tr>
					<%                                                               
                            }
