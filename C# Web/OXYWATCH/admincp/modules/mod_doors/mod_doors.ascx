<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_doors.ascx.cs" Inherits="admincp_modules_mod_doors_mod_doors" %>
...
function checkSubmitActiveInactiveForm(field, strAction)
{
	...
	else
	{
	    ...
	    {	    
	        ...
	        field.action = "Default.aspx?page=doors&mod=doors&do=" + strAction;
		    field.submit();
		    return true;
		}
		else
...
/*
    ...
    */
    field.action="Default.aspx?page=doors&mod=doors&do=search&key="+field.key.value+"&per="+field.per.value;
    field.submit();
    return true;
}
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<td width="95%"><span class="title">Quản lý cửa nhựa, cửa cuốn</span></td>
				</tr>    		
			</table>
				<table class="content">
					<tr>
						...
						<table class="header" cellpadding="0" cellspacing="0">
							<tr>
								<th>Danh sách các cửa nhựa, cửa cuốn</th>
							</tr>	
						</table>
						</td>
					</tr>
					<tr>
						...
						<table class="search">
							<tr>
								...
								<%= clsHtml.add(2, "Default.aspx?page=add_edit_doors&mod=doors&do=add")%>									
								</td>
							</tr>
						</table>
						</div>
						<table class="table" cellSpacing="1" cellPadding="1">		
							<tr>
								...
								<th  width="20%" >Tên doors</th>
								<th  width="20%" >Giới thiệu ngắn</th>
					...
					<%	
                        ...
                        string strSearch = "";
                        if (strKey != null)
                            strSearch = " and (tbl_doors.C_Name like '%" + strKey + "%' )";                        		    
                        //------------------------------ 		    
                        string strSqlCount = "select count(*) from tbl_doors inner join tbl_category_news on tbl_doors.FK_CategoryID = tbl_category_news.PK_CategoryID where tbl_doors.FK_LangID = " + lang.getLangID() + strSearch;
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select tbl_doors.*, tbl_category_news.C_Name as CategoryName from tbl_doors inner join tbl_category_news on tbl_doors.FK_CategoryID = tbl_category_news.PK_CategoryID where tbl_doors.FK_LangID = " + lang.getLangID() + strSearch + " order by tbl_doors.C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light">
								<td align="center">
								<% Response.Write("<input name=\"row[]\" type=\"checkbox\" class=\"input_checkbox\" id=\"row[]\" value=\"" + dt.Rows[i]["PK_doorsID"].ToString() + "\" />"); %>								
								</td>
								<td align="center">		
								...
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_doors", "tbl_doors.FK_LangID =" + lang.getLangID()) > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=up&id=" + dt.Rows[i]["PK_doorsID"].ToString() + "'>"); %>
								    <img src="images/arrow-up.gif"  onmouseover="Tip('Di chuyển lên')" border="0">
								    <%= "</a>" %>
		                        <%
                                    }		                         
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_doors", "tbl_doors.FK_LangID =" + lang.getLangID()) < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=down&id=" + dt.Rows[i]["PK_doorsID"].ToString() + "'>"); %>
								...
								<% if ((int)dt.Rows[i]["C_Active"] == 0)
                                   {
                                %>
								   <%= clsHtml.UnLock(2, clsConfig.getCurrentUrl() + "&do=unlock&id=" + dt.Rows[i]["PK_doorsID"].ToString())%>
		                        <%
                                    }
                                    else
                                    {		                         
                                %>
                                <%= clsHtml.Lock(2, clsConfig.getCurrentUrl() + "&do=lock&id=" + dt.Rows[i]["PK_doorsID"].ToString())%>
								<% } %>									
								&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_doors&mod=doors&do=edit&id=" + dt.Rows[i]["PK_doorsID"].ToString())%> 
								&nbsp;								
								<%= clsHtml.del(2, clsConfig.getCurrentUrl() + "&do=delete&id=" + dt.Rows[i]["PK_doorsID"].ToString())%>    							
								</td>	
							</tr>
					<%                                                               
                            }
