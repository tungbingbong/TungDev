<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_hoadon.ascx.cs" Inherits="admincp_modules_mod_hoadon_mod_hoadon" %>
...
<script type="text/javascript">
    function checkSubmitActiveInactiveForm(field, strAction) {
        ...
        if (!isChecked) {
            ...
            if (confirm('Ban da chac chan thuc hien tac vu nay chua?')) {
                ...
                field.action = "Default.aspx?page=hoadon&mod=hoadon&do=" + strAction;
                field.submit();
                return true;
            }
            else
    ...
    function checkFormSearch(field) {
        ...
        */
        ...
        obj.style.visibility = (obj.style.visibility == 'visible') ? 'hidden' : 'visible';
        //=================================================
        field.action = "Default.aspx?page=hoadon&mod=hoadon&do=search&key=" + field.key.value + "&per=" + field.per.value;
        field.submit();
        return true;
    }
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<td width="95%"><span class="title">Quản lý khách hàng</span></td>
				</tr>    		
			</table>
				<table class="content">
					<tr>
					...
					<tr>
						...
						<table class="search">
							<tr>
								...
								<%--<%= clsHtml.add(2, "Default.aspx?page=add_edit_hoadon&mod=hoadon&do=add")%>	--%>								
								</td>
							</tr>
						</table>
						</div>
					...
					<%	
                        ...
                        //------------------------------	
                        int FK_GroupMemberID = clsInput.getNumericInput("type",0);
                        ...
                        string strSearch = "";
                        if (strKey != null)
                            strSearch = " and (C_CustomerName like '%" + strKey + "%' or C_FullName like '%" + strKey + "%' or C_Email like '%" + strKey + "%' or C_Address like '%" + strKey + "%' or C_Phone like '%" + strKey + "%' )";
                        ...
                        //}
                        //------------------------------ 		    
                        string strSqlCount = "select count(*) from tbl_customer_cart where (FK_LangID = 1 or FK_LangID = 2)" + strSearch;
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select * from tbl_customer_cart where  (FK_LangID = 1 or FK_LangID = 2)" + strSearch + " order by C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light">
								<td align="center">
								<% Response.Write("<input name=\"row[]\" type=\"checkbox\" class=\"input_checkbox\" id=\"row[]\" value=\"" + dt.Rows[i]["PK_CustomerID"].ToString() + "\" />"); %>								
								</td>
								<td align="left">								
								<img src="images/arrow.gif">								
								&nbsp;
								<%= dt.Rows[i]["C_CustomerName"].ToString() %>
								</td>								
								<%--<td align="left">														
								&nbsp;
                                ...
                                <td align="left">														
								&nbsp;
								<%= dt.Rows[i]["C_Phone"].ToString() %>
								</td>		
                                <td align="left">														
								&nbsp;
								<%= DateTime.Parse(dt.Rows[i]["C_Create"].ToString()).ToString("dd/MM/yyyy")%>
								</td>	
                                <td align="left">														
								&nbsp;
								<%= dt.Rows[i]["C_Code"].ToString() %>
								</td>		
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_customer_cart", "") > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=up&id=" + dt.Rows[i]["PK_CustomerID"].ToString() + "'>"); %>
								    <img src="images/arrow-up.gif"  onmouseover="Tip('Di chuyển lên')" border="0">
								    <%= "</a>" %>
		                        <%
                                    }		                         
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_customer_cart", "") < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=down&id=" + dt.Rows[i]["PK_CustomerID"].ToString() + "'>"); %>
								    <img src="images/arrow-down.gif"  onmouseover="Tip('Di chuyển xuống')" border="0">
								    <%= "</a>"%>
		                        <% } %>
								</td>
								<td align="center">
								<%= clsHtml.view(2, "Default.aspx?page=view_hoadon&mod=hoadon&id=" + dt.Rows[i]["PK_CustomerID"].ToString())%>
								&nbsp;
								<% if ((int)dt.Rows[i]["C_Active"] == 0)
                                   {
                                %>
								   <%= clsHtml.UnLock(2, clsConfig.getCurrentUrl() + "&do=unlock&id=" + dt.Rows[i]["PK_CustomerID"].ToString())%>
		                        <%
                                    }
                                    else
                                    {		                         
                                %>
                                <%= clsHtml.Lock(2, clsConfig.getCurrentUrl() + "&do=lock&id=" + dt.Rows[i]["PK_CustomerID"].ToString())%>
								<% } %>		
                                							
								
                                &nbsp;								
								<%= clsHtml.del(2, clsConfig.getCurrentUrl() + "&do=delete&id=" + dt.Rows[i]["PK_CustomerID"].ToString())%>    							
								</td>	
							</tr>
					<%                                                               
                            }
