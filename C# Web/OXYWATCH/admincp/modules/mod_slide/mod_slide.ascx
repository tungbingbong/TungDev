<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_slide.ascx.cs" Inherits="admincp_modules_mod_slide_mod_slide" %>
...
<script type="text/javascript">
    function checkSubmitActiveInactiveForm(field, strAction) {
        ...
        if (!isChecked) {
            ...
            if (confirm('Ban da chac chan thuc hien tac vu nay chua?')) {
                ...
                field.action = "Default.aspx?page=slide&mod=slide&do=" + strAction;
                field.submit();
                return true;
            }
            else
    ...
    function checkFormSearch(field) {
        ...
        */
        field.action = "Default.aspx?page=slide&mod=slide&do=search&key=" + field.key.value + "&per=" + field.per.value;
        field.submit();
        return true;
    }
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					<td width="95%"><span class="title">Quản lý các slide</span></td>
				</tr>    		
			</table>
				<table class="content">
					<tr>
					...
					<tr>
						...
						<table class="search">
							<tr>
								...
								<%= clsHtml.add(2, "Default.aspx?page=add_edit_slide&mod=slide&do=add")%>									
								</td>
							</tr>
						</table>
						</div>
					...
					<%	
                        ...
                        string strSearch = "";
                        if (strKey != null)
                            strSearch = " and (tbl_slide.C_Name like '%" + strKey + "%' )";                        		    
                        //------------------------------ 		    
                        //v1
                        string strSqlCount = "select count(*) from tbl_slide where tbl_slide.FK_LangID = " + lang.getLangID() + strSearch;
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select *,(Select C_Name from tbl_category_slide where tbl_category_slide.PK_CategoryID=tbl_slide.FK_CategoryID) AS TenLoaiSlide from tbl_slide where tbl_slide.FK_LangID = " + lang.getLangID() + strSearch + " order by tbl_slide.C_Rank desc";
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light">
								<td align="center">
								<% Response.Write("<input name=\"row[]\" type=\"checkbox\" class=\"input_checkbox\" id=\"row[]\" value=\"" + dt.Rows[i]["PK_slideID"].ToString() + "\" />"); %>								
								</td>
								<td align="center">		
                                ...
                                <td align="center">														
								&nbsp;
								<%= dt.Rows[i]["TenLoaiSlide"].ToString()%>

								</td>	
                                		
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_slide", "tbl_slide.FK_LangID =" + lang.getLangID()) > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=up&id=" + dt.Rows[i]["PK_slideID"].ToString() + "'>"); %>
								    <img src="images/arrow-up.gif"  onmouseover="Tip('Di chuyển lên')" border="0">
								    <%= "</a>" %>
		                        <%
                                    }		                         
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_slide", "tbl_slide.FK_LangID =" + lang.getLangID()) < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								    <% Response.Write("<a href='" + clsConfig.getCurrentUrl() + "&do=down&id=" + dt.Rows[i]["PK_slideID"].ToString() + "'>"); %>
								...
								<% if ((int)dt.Rows[i]["C_Active"] == 0)
                                   {
                                %>
								   <%= clsHtml.UnLock(2, clsConfig.getCurrentUrl() + "&do=unlock&id=" + dt.Rows[i]["PK_slideID"].ToString())%>
		                        <%
                                    }
                                    else
                                    {		                         
                                %>
                                <%= clsHtml.Lock(2, clsConfig.getCurrentUrl() + "&do=lock&id=" + dt.Rows[i]["PK_slideID"].ToString())%>
								<% } %>									
								&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_slide&mod=slide&do=edit&id=" + dt.Rows[i]["PK_slideID"].ToString())%> 
								&nbsp;								
								<%= clsHtml.del(2, clsConfig.getCurrentUrl() + "&do=delete&id=" + dt.Rows[i]["PK_slideID"].ToString())%>    							
								</td>	
							</tr>
					<%                                                               
                            }
