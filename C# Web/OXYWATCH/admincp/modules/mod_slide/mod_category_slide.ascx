<%@ Control Language="C#" AutoEventWireup="true" CodeFile="mod_category_slide.ascx.cs" Inherits="admincp_modules_mod_slide_mod_category_slide" %>
		...
		<tr>
			...
			<tr>
				...
				<tr>
					...
					-->					
					<%= clsHtml.add(2, "Default.aspx?page=add_edit_category_slide&mod=slide&do=add")%>
					</td>
				...
				<table class="content">
					...
					<%	
                        ...
                        //------------------------------ 
                        string strSqlCount = "select count(*) from tbl_category_slide where 1=1 AND FK_LangID = " + lang.getLangID();
                        ...
                        //========================================		    		                             	    	               			        		    
                        string strSql = "select * from tbl_category_slide where 1=1 AND FK_LangID = " + lang.getLangID() + " order by C_Rank desc";
                        DataTable dt;
                        ...
                        if (dt.Rows.Count > 0)
                        {
					     ...
					     %>
							<tr class="light" >	
								...
								<td align="center">
								<% if (clsSwap.getMaxRankRecord("tbl_category_slide", "tbl_category_slide.FK_LangID =" + lang.getLangID()) > (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
                                ...
                                %>
								<% if (clsSwap.getMinRankRecord("tbl_category_slide", "tbl_category_slide.FK_LangID =" + lang.getLangID()) < (int)dt.Rows[i]["C_Rank"])
                                    {
                                %>
								...
								<% } %>									
								&nbsp;							
								<%= clsHtml.edit(2, "Default.aspx?page=add_edit_category_slide&mod=slide&do=edit&id=" + dt.Rows[i]["PK_CategoryID"].ToString())%> 
								&nbsp;								
