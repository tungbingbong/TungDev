...
public partial class admincp_modules_mod_slide_mod_add_edit_category_slide : System.Web.UI.UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            //==================================================
            //Display Msg
            clsConfig.redirectUrl("Default.aspx?page=category_slide&mod=slide");
            //==================================================
        }
    ...
    private void edit_category_slide(int intID)
    {
        DataTable dt = clsDatabase.getDataTable("select * from tbl_category_slide where PK_CategoryID = " + intID);
        string strName = "";
        string strDes = "";
        int intCheckUrl = 0;
        string strUrl = "";
    ...
    private void add_category_slide()
    {
    ...
    protected void BtnSubmit_ServerClick(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                string strSql = "insert into tbl_category_slide(FK_LangID, C_Name, C_Des, C_Active) values (" + lang.getLangID() + ",N'" + strName  + "',N'" + strDes + "',"  + "1)";
                clsDatabase.ExecuteQuery(strSql);
                int intInsertRecord = clsDatabase.getInsertID("tbl_category_slide", "PK_CategoryID");
                clsDatabase.ExecuteQuery("update tbl_category_slide set C_Rank = " + intInsertRecord + " where PK_CategoryID = " + intInsertRecord);
                Response.Redirect("Default.aspx?page=category_slide&mod=slide");
            }
            catch
            {
    ...
    protected void BtnEdit_ServerClick(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                string strSql = "update tbl_category_slide set C_Name = N'" + strName + "', C_Des = N'" + strDes + "'" + " where PK_CategoryID=" + intId;
                clsDatabase.ExecuteQuery(strSql);
                Response.Redirect("Default.aspx?page=category_slide&mod=slide");
            }
            catch
