					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="col-sm-16 text-center"><a {TITLE_ORDER} href="{URL_title}"><strong>{LANG.template_title}</strong></a> </td>
						...
						<tbody>
							...
							<tr id="group_{LOOP.template_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.template_id}" >
								</td>
								<td class="text-center">
									<select id="id_weight_{LOOP.template_id}"  class="form-control formajax" data-action="weight" data-id="{LOOP.template_id}" data-token="{LOOP.token}">
									<!-- BEGIN: weight -->
	...
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#template-content').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '<i class="fa fa-times"></i></div></div>');
				}
				
				if (json['success']) {
					$('#template-content').prepend('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '<i class="fa fa-times"></i></div></div>');
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=template&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: 'action=' + action + '&template_id=' + template_id + '&new_vid=' + new_vid + '&token='+token,
		beforeSend: function() {
