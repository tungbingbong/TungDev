					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="col-sm-14 text-center"><a {TITLE_ORDER} href="{URL_title}"><strong>{LANG.group_title}</strong></a> </td>
								<td class="col-sm-6 text-center"> <strong>{LANG.action} </strong></td>
							</tr>
						</thead>
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.group_id}">
								...
								<td class="text-center">
									<select id="id_weight_{LOOP.group_id}"  class="form-control formajax" data-action="weight" data-id="{LOOP.group_id}" data-token="{LOOP.token}">
									<!-- BEGIN: weight -->
								...
								<td class="text-center">
									<a href="{LOOP.add}" data-toggle="tooltip" title="{LANG.group_add_photo}" class="btn btn-success"><i class="fa fa-plus"></i></a>
									<a href="{LOOP.view}" data-toggle="tooltip" title="{LANG.group_view_photo}" class="btn btn-info"><i class="fa fa-eye"></i></a>
	...
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#group-content').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '<i class="fa fa-times"></i></div></div>');
				}
				
				if (json['success']) {
					$('#group-content').prepend('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '<i class="fa fa-times"></i></div></div>');
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=group&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: 'action=' + action + '&group_id=' + group_id + '&new_vid=' + new_vid + '&token='+token,
		beforeSend: function() {
