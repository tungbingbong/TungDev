	...
	{ 
		...
		foreach( $global_photo_cat as $_category_id => $category  )
		{
			if( $category['parent_id'] == 0 and $category['inhome'] != 0 and $category['status'] != 0 )
			{
				...
				$db->sqlreset()
					->select( 'a.album_id, a.category_id, a.name, a.model, model, a.alias, a.capturelocal, a.description, a.num_photo, a.date_added, a.viewed, a.author, r.file, r.thumb' )
				...
				$array_content = array();
				while( list( $album_id, $category_id, $name, $model, $alias, $capturelocal, $description, $num_photo, $date_added, $viewed, $author_id, $file, $thumb ) = $result->fetch( 3 ) )
				{
...
elseif( $module_config[$module_name]['home_view'] == 'home_view_grid_by_album' )
{
	...
	{ 
		...
		$db->select( 'a.album_id, a.name, a.model, model, a.category_id, a.alias, a.capturelocal, a.description, a.num_photo, a.date_added, a.viewed, a.author, r.file, r.thumb' )
