...
function template_fix_weight()
{
	global $db_slave;
	$sql = 'SELECT template_id FROM ' . TABLE_SLIDER_NAME . '_template ORDER BY weight ASC';
	$result = $db_slave->query( $sql );
	$weight = 0;
	while( $row = $result->fetch() )
	{
		++$weight;
		$db_slave->query( 'UPDATE ' . TABLE_SLIDER_NAME . '_template SET weight=' . $weight . ' WHERE template_id=' . $row['template_id'] );
	}
	$result->closeCursor();
}
...
if( ACTION_METHOD == 'delete' )
{
	...
	elseif( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $template_id ) )
	{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $template_id )
		{
			...
			$db_slave->query( 'DELETE FROM ' . TABLE_SLIDER_NAME . '_template WHERE template_id = ' . ( int )$template_id );

			$json['id'][$a] = $template_id;

			$_del_array[] = $template_id;
		...
		if( $count )
		{
			...
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_template', implode( ', ', $_del_array ), $admin_info['userid'] );

			$nv_Cache->delMod($module_name);

			$json['success'] = $lang_module['template_delete_success'];
		}

	}
	else
	{
		$json['error'] = $lang_module['template_error_security'];
	}
...
elseif( ACTION_METHOD == 'status' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $template_id ) )
	{
		$sql = 'UPDATE ' . TABLE_SLIDER_NAME . '_template SET status=' . $new_vid . ' WHERE template_id=' . $template_id;
		if( $db_slave->exec( $sql ) )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_change_status_template', 'template_id:' . $template_id, $admin_info['userid'] );

			$nv_Cache->delMod($module_name);

			$json['success'] = $lang_module['template_status_success'];

		}
		else
		{
			$json['error'] = $lang_module['template_error_status'];
...
elseif( ACTION_METHOD == 'weight' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $template_id ) )
	{
		$sql = 'SELECT template_id FROM ' . TABLE_SLIDER_NAME . '_template WHERE template_id!=' . $template_id . ' ORDER BY weight ASC';
		$result = $db_slave->query( $sql );
		...
		while( $row = $result->fetch() )
		{
			...
			$sql = 'UPDATE ' . TABLE_SLIDER_NAME . '_template SET weight=' . $weight . ' WHERE template_id=' . intval( $row['template_id'] );
			$db_slave->query( $sql );
		}

		$sql = 'UPDATE ' . TABLE_SLIDER_NAME . '_template SET weight=' . $new_vid . ' WHERE template_id=' . $template_id;
		if( $db_slave->exec( $sql ) )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_change_weight_template', 'template_id:' . $template_id, $admin_info['userid'] );

			$nv_Cache->delMod($module_name);

			$json['success'] = $lang_module['template_weight_success'];
			$json['link'] = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op;

		}
		else
		{
			$json['error'] = $lang_module['template_error_weight'];
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		if( empty( $data['title'] ) )
		{
			$error['title'] = $lang_module['template_error_title'];
		}else
		{
			if( ! preg_match('/^[a-z0-9.\_]+$/i', $data['title'] ) )
			{
				$error['title'] = $lang_module['template_help'];
			}
		...
		if( empty( $error ) )
		{
			if( $data['template_id'] == 0 )
			{
				try
				{
					$stmt = $db_slave->prepare( 'SELECT MAX(weight) FROM ' . TABLE_SLIDER_NAME . '_template' );
					$stmt->execute();
					$weight = $stmt->fetchColumn();

					$weight = intval( $weight ) + 1;

					$stmt = $db_slave->prepare( 'INSERT INTO ' . TABLE_SLIDER_NAME . '_template SET 
					...
					if( $data['template_id'] = $db_slave->lastInsertId() )
					{
						
						$nv_Request->set_Session( $module_data . '_success', $lang_module['template_insert_success'] );
						
						nv_insert_logs( NV_LANG_DATA, $module_name, 'log_add_template', 'template_id: ' . $data['template_id'], $admin_info['userid'] );
			...
			else
			{
				try
				{
					...
					$stmt = $db_slave->prepare( 'UPDATE ' . TABLE_SLIDER_NAME . '_template SET 
						title =:title 
					...
					if( $stmt->execute() )
					{
						$nv_Request->set_Session( $module_data . '_success', $lang_module['template_update_success'] );
						
						nv_insert_logs( NV_LANG_DATA, $module_name, 'log_edit_template', 'template_id: ' . $data['template_id'], $admin_info['userid'] );
					}
					else
					{
						$error['warning'] = $lang_module['template_error_save'];
...
$xtpl->assign( 'ADD_NEW', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=template&action=add' );
...
if( ! empty( $array ) )
{
	foreach( $array as $item )
	{
		...
		$item['token'] = md5( $nv_Request->session_id . $global_config['sitekey'] . $item['template_id'] );
		$item['edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&action=edit&token=' . $item['token'] . '&template_id=' . $item['template_id'];
