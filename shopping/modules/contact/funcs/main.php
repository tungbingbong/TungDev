...
if (defined('NV_IS_USER')) {
	...
	$fgoidichvu = isset($user_info['goidichvu']) ? $user_info['goidichvu'] : '';
	$fkhuvuc = isset($user_info['khuvuc']) ? $user_info['khuvuc'] : '';
	$fdientich = isset($user_info['dientich']) ? $user_info['dientich'] : '';
}
...
if ($nv_Request->isset_request('checkss', 'post')) {
	...
	$fgoidichvu = nv_substr($nv_Request->get_title('fgoidichvu', 'post', '', 1), 0, 255);
	$fkhuvuc = nv_substr($nv_Request->get_title('fkhuvuc', 'post', '', 1), 0, 250);
	$fdientich = nv_substr($nv_Request->get_title('fdientich', 'post', '', 1), 0, 200);
    ...
    (cid, cat, title, content, send_time, sender_id, sender_name, sender_email, sender_phone, sender_goidichvu, sender_khuvuc, sender_dientich, sender_ip, is_read, is_reply) VALUES
    (' . $fpart . ', :cat, :title, :content, ' . NV_CURRENTTIME . ', ' . $sender_id . ', :sender_name, :sender_email, :sender_phone, :sender_goidichvu, :sender_khuvuc, :sender_dientich, :sender_ip, 0, 0)';
    $data_insert = array();
	...
	$data_insert['sender_goidichvu'] = $fgoidichvu;
	$data_insert['sender_khuvuc'] = $fkhuvuc;
	$data_insert['sender_dientich'] = $fdientich;
    $data_insert['sender_ip'] = $client_info['ip'];
    $row_id = $db->insert_id($sql, 'id', $data_insert);
    if ($row_id > 0) {
    	$fcon_mail = contact_sendcontact( $row_id, $fcat, $ftitle, $fname, $femail, $fphone, $fgoidichvu, $fkhuvuc, $fdientich, $fcon, $fpart );
		...
		if ($fsendcopy) {
			...
			$fcon_mail = contact_sendcontact( $row_id, $fcat, $ftitle, $fname, $femail, $fphone, $fgoidichvu, $fkhuvuc, $fdientich, $fcon, $fpart, false );
